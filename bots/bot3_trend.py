...
def get_signal(closes_1h, closes_4h, weights):
    ...
    reasons = []
    if cur > e20 > e50 > e100:
        score += 3.0; reasons.append("Strong 4H bull trend")
    elif cur < e20 < e50 < e100:
        score -= 3.0; reasons.append("Strong 4H bear trend")
    elif cur > e20 > e50:
        score += 1.5; reasons.append("4H bull trend")
    elif cur < e20 < e50:
        score -= 1.5; reasons.append("4H bear trend")
    if slope > 0.03: score += 1.5; reasons.append(f"Upslope +{slope*100:.1f}%")
    elif slope > 0.01: score += 0.7
    elif slope < -0.03: score -= 1.5; reasons.append(f"Downslope {slope*100:.1f}%")
    elif slope < -0.01: score -= 0.7
    if e9_1h > e21_1h and rsi < 65: score += 1.0; reasons.append("1H confirms")
    elif e9_1h < e21_1h and rsi > 35: score -= 1.0
    if rsi > 75: score -= 1.5; reasons.append("Overbought")
    if rsi < 25: score += 1.5; reasons.append("Oversold")
...
def run_bot():
    ...
    for symbol in SYMBOLS:
        try:
            bars_1h = data_client.get_crypto_bars(CryptoBarsRequest(
                symbol_or_symbols=symbol, timeframe=TimeFrame.Hour, limit=60))
            bars_4h = data_client.get_crypto_bars(CryptoBarsRequest(
            ...
            cur = closes_1h[-1]
            score, sig, reason = get_signal(closes_1h, closes_4h, weights)
                ...
                if spend > 20:
                    ...
                    try:
                        ...
                        t = {"bot_id":BOT_ID,"symbol":symbol,"action":"BUY",
                             ...
                             "strategy":"TREND","timestamp":datetime.now(timezone.utc).isoformat()}
