...
def get_signal(closes, weights):
    ...
    if e9 > e21 > e50: score += 2.0; reasons.append("Bullish EMA")
    elif e9 < e21 < e50: score -= 2.0; reasons.append("Bearish EMA")
    if macd > 0: score += 1.0; reasons.append("MACD+")
    else: score -= 1.0; reasons.append("MACD-")
    ...
    for symbol in SYMBOLS:
        try:
            bars = data.get_crypto_bars(CryptoBarsRequest(
                symbol_or_symbols=symbol, timeframe=TimeFrame.Hour, limit=100))
                ...
                if spend > 20:
                    ...
                    try:
                        ...
                        t = {"bot_id":BOT_ID,"symbol":symbol,"action":"BUY",
                             ...
                             "strategy":"MOMENTUM","timestamp":datetime.now(timezone.utc).isoformat()}
