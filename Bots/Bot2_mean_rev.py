...
def signal(closes, weights):
    ...
    score = 0.0
    reasons = []
    if cur < lower: score += 3.0; reasons.append(f"Below BB lower")
    ...
    if zscore < -2.0: score += 2.0; reasons.append(f"Z-score extreme low {zscore:.2f}")
    elif zscore < -1.5: score += 1.0
    elif zscore > 2.0: score -= 2.0; reasons.append(f"Z-score extreme high {zscore:.2f}")
    elif zscore > 1.5: score -= 1.0
    if rsi < 25: score += 1.5; reasons.append(f"RSI oversold {rsi:.0f}")
    elif rsi < 35: score += 0.7
    elif rsi > 75: score -= 1.5; reasons.append(f"RSI overbought {rsi:.0f}")
...
def run_bot():
    ...
    for symbol in SYMBOLS:
        try:
            req = CryptoBarsRequest(symbol_or_symbols=symbol, timeframe=TimeFrame.Hour, limit=80)
            ...
            if sig == "BUY" and not held and cash > 30:
                ...
                try:
                    ...
                    trade = {"bot_id": BOT_ID, "symbol": symbol, "action": "SELL",
                             ...
                             "profit": profit, "reason": reason, "strategy": "MEAN_REV",
                             "timestamp": datetime.now(timezone.utc).isoformat()}
                    db.save_trade(trade)
